...
class GRSCaptcha {
	...
	public function reloadCaptcha() {
		$_SESSION['captcha'] = self::simple_php_captcha();
		session_write_close();
		$this->ajaxClass->result('success', $_SESSION['captcha']['image_src']);
	}
	...
	public static function simple_php_captcha($config = array()) {
		...
		if( !function_exists('gd_info') ) {
			throw new Exception(__('Required GD library is missing', 'limb-gallery'));
		}
		$bg_path = GRS_PLG_DIR . '/css/captcha/backgrounds/';
		$font_path = GRS_PLG_DIR . '/css/captcha/fonts/';
		// Default values
		...
		$image_src = add_query_arg( array("action" => "grsGalleryAjax", "grsAction" => "captcha", "_CAPTCHA" => "","t" => urlencode(microtime())), admin_url("admin-ajax.php"));
